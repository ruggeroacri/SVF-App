...
module.exports = async (req, res) => {
  try {
    ...
    $('.match-list__match').each((_, el) => {
      if (matches.length >= 50) return false; // hard limit to avoid large payloads
      const home = pick($(el).find('.match-list__team--home .match-list__team-name').text());
      const away = pick($(el).find('.match-list__team--away .match-list__team-name').text());
      const date = pick($(el).find('.match-list__date').text());
      const result = pick($(el).find('.match-list__result').text());
